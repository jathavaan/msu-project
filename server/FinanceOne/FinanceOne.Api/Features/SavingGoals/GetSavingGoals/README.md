...
- Also includes `InterestRate`, the goal's own field (nullable annual percentage) — `null` when the goal has no rate set, in which case it behaves exactly as before this field existed. See `Features/SavingGoals/GetSavingGoalProjection` for how it's used to project a goal's balance forward.
- Also includes `ImageUrl` — `null` until an image is uploaded via Upload Saving Goal Image, after which it is `/api/saving-goals/{id}/image` (see Get Saving Goal Image).
